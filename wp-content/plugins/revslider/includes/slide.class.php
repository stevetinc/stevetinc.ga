...
class RevSliderSlide extends RevSliderFunctions {
	 ...
	 * @change 6.2.16: $template_id will not be written if the current post id is not the post_id to prevent malfunctioning
	 */
	public function init_by_post_data($data, RevSliderSlide $template, $slider_id){
		...
		$template_id = ($template_id == '' || (intval($c_post_id) !== 0 && intval($post_id) !== intval($c_post_id))) ? 'default' : $template_id;
